...
- **三态首页**：进行中显示今天（高亮）与明天（虚线预览）；期前倒计时；期后提示导入新表。餐次默认收起不配图，点一下展开（配图预留 + 做法 + 菜品详情）
- **一周 / 单日视图**：按餐次配色（早餐米黄 / 午餐绿 / 加餐粉 / 晚餐蓝），备料备注早晚分栏；一周页同样「点开看」，单日页全展开。一周页的星期导航常驻吸顶：滚动时大标题上移滚出，导航始终可见，并随滚动自动高亮「现在看的是周几」
